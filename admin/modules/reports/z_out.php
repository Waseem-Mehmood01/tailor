...
$sql_net_gifts = "SELECT COALESCE(SUM(si.`total_amount`), 0) AS total_amount FROM sale_invoice si
WHERE si.`sale_invoice_id` NOT IN (SELECT sg.`sales_invoice_id`
                       FROM sale_split_gift sg) ".$where_clause." AND si.`tender` = 'gift' AND si.`is_reverse` = '0' ";
$gift_net = DB::queryFirstField($sql_net_gifts);

$printAble .='<tr>
<td></td>
<td></td>
<td><h5><strong>Gifts<strong></h5></td>
<td></td>
</tr>
<tr>
<td></td>
...
$sql_gifts = "SELECT COALESCE(SUM(sg.`amount`), 0) AS total_amount FROM sale_invoice si, sale_split_gift sg
WHERE 1=1 ".$where_clause." AND sg.`sales_invoice_id` = si.`sale_invoice_id` AND sg.`payment_type`='gift' AND si.`tender` = 'gift' AND si.`is_reverse` = '0' ";
$gift_gift = DB::queryFirstField($sql_gifts);
...
<td></td>
<td></td>
<td><b>Net Gift Cert : </b></td>
<td><b>'.$total_gifts.'</b></td>
</tr>
<tr>
<td></td>
<td></td>
...
$sql_amount = "SELECT COALESCE(SUM(si.`tender_amount`), 0) AS total_amount  FROM sale_invoice si
		WHERE 1=1 ".$where_clause."  AND `is_reverse`='0' AND `tender`='cash' AND `reciept_type`='sale'";
