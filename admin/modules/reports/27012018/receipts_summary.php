...
$sql = "SELECT * FROM sale_invoice si 
		WHERE 1=1 ".$where_clause." AND si.`is_reverse`='0' 
...
if($row['reciept_type'] == 'return'){
    ...
    $sub_price = $row['sub_total'] * -1;
    $qty = $row['total_qty'] * -1;
} else {
    $total_price = $row['total_amount'];
    $qty = $row['total_qty'];
    $sub_price = $row['sub_total']; 
}
...
}else if($row['reciept_type'] <> 'return'){
    $total_price = $row['total_amount'];
    $qty = $row['total_qty'];
    $sub_price = $row['sub_total'];
    $txt_rev = ' ';
...
$tbl->addCell(strtoupper($row['reciept_type']).$txt_rev);
