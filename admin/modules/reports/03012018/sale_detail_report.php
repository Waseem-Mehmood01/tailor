...
$ded_reverse = 0.00;
$sql_ded = "SELECT sd.`item`, sd.`dcs` ,sd.`description`, si.*
		 FROM sale_invoice_detail sd
				LEFT JOIN sale_invoice si
				ON(si.`sale_invoice_id`=sd.`sale_invoice_id` AND si.`reciept_type`='sale'  AND si.`is_reverse`='1')
				WHERE 1=1 ".$where_clause."
				 GROUP BY  si.`sale_invoice_id` ".$having_clause." ORDER BY si.`sale_invoice_id` DESC";
$deductions_row = DB::query($sql_ded);
foreach($deductions_row as $deductions_rows){
	$ded_reverse += $deductions_rows['total_amount'];
}
$ded_returns = 0.00;
$sql_ret = "SELECT sd.`item`, sd.`dcs` ,sd.`description`, si.*
		 FROM sale_invoice_detail sd
				LEFT JOIN sale_invoice si
...
foreach($returns_row as $returns_rows){
	$ded_returns += $returns_rows['total_amount'];
} 
