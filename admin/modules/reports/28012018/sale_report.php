...
$tbl->addCell('DCS', '', 'header');
$tbl->addCell('Desc', '', 'header');
...
$tbl->addCell('Discount (USD)', '', 'header');
$tbl->addCell('Tax %', '', 'header');
$tbl->addCell('Tax (USD)', '', 'header');
$tbl->addCell('Shipping Cost Total', '', 'header');
...
$sql = "SELECT sd.`item`, sd.`dcs` ,sd.`description`,si.`sale_invoice_id`,  si.`reciept_workstation`, si.`reciept_type`,si.`tender`,si.`is_reverse`, si.`created_by`,  
		...
		SUM(si.`ship_amount`) AS total_shiping
		 FROM sale_invoice_detail sd 
				LEFT JOIN sale_invoice si
				ON(si.`sale_invoice_id`=sd.`sale_invoice_id` AND si.`reciept_type`='sale' AND si.`tender`!='gift'
                  AND si.`is_reverse`='0') 
        ...
        foreach ($res1 as $row){ 
           /* 
            if($row['tender'] == 'gift') { 
                ...
                $g_cash_r = DB::query("select payment_type, amount from sale_split_gift where sales_invoice_id='".$row['sale_invoice_id']."'");
                if(!empty($g_cash_r)){
                    foreach($g_cash_r as $g_cash_row){
                        if($g_cash_row['payment_type']=='cash'){
                            $gift_cash += $g_cash_row['amount'];
                        } else {
                            $gift_gift += $g_cash_row['amount'];
                        }
                    }
                } else {
                    $ded_gift +=$row['total_sale'];
                }
                
            } */
            $sub_total +=$row['total_sale'];
...
foreach($res as $row) {
$get_tax = $row['total_sale'] / 1.05;
$tax_cal = round($row['total_sale'] - $get_tax, 2);	
$tend = $row['tender'];
...
$tbl->addCell($row['total_dis_perc']);
$tbl->addCell($row['total_dis']);
$tbl->addCell(5);
$tbl->addCell($tax_cal);
$tbl->addCell($row['total_shiping']);
$tbl->addCell(strtoupper($tend));
$tbl->addCell('<b>'.$row['total_sale'].'</b>');
 
}



$sql_retur = "SELECT sd.`item`, sd.`dcs` ,sd.`description`, si.`sale_invoice_id`, si.`reciept_workstation`, si.`reciept_type`,si.`tender`,si.`is_reverse`, si.`created_by`,
		MAX(si.`created_on`) AS last_sold_on,
		SUM(sd.`total`) AS total_sale, SUM(sd.`qty`) AS total_qty,
		SUM(si.`dis_perc`) AS total_dis_perc, SUM(si.`dis_amount`) AS total_dis,
		SUM(si.`tax_perc`) AS total_tax_perc, SUM(si.`tax_amount`) AS total_tax,
		SUM(si.`ship_amount`) AS total_shiping
		 FROM sale_invoice_detail sd
				LEFT JOIN sale_invoice si
				ON(si.`sale_invoice_id`=sd.`sale_invoice_id` AND si.`reciept_type`='return'
                  AND si.`is_reverse`='0')
				WHERE 1=1 ".$where_clause."
				 GROUP BY sd.`item` ".$having_clause." ORDER BY last_sold_on DESC";
...
$sql_retur .= ' LIMIT '.$pageLimit.', '.$setLimit;
...
$res_ret = DB::query($sql_retur);
$get_tax = 0.00;
$tax_cal = 0.00;

foreach($res_ret as $row) {
    $get_tax = $row['total_sale'] / 1.05;
    $tax_cal = round($row['total_sale'] - $get_tax, 2);
    ...
    $tbl->addCell($row['dcs']);
    $tbl->addCell(stripslashes($row['description']));
    $tbl->addCell($row['total_qty']*-1);
    $tbl->addCell(getDateTime($row['last_sold_on'], "dtShort"));
    $tbl->addCell(get_store_workstation_name($row['reciept_workstation']));
    $tbl->addCell($row['created_by']);
    $tbl->addCell($row['total_dis_perc']);
    $tbl->addCell($row['total_dis']*-1);
    $tbl->addCell(5);
    $tbl->addCell($tax_cal*-1);
    $tbl->addCell($row['total_shiping']*-1);
    $tbl->addCell(strtoupper('Return'));
    $tbl->addCell('<b>'.($row['total_sale']*-1).'</b>');
...
$ded_reverse = 0.00;
$sql_ded = "SELECT sd.`item`, sd.`dcs` ,sd.`description`,  si.`reciept_workstation`, si.`reciept_type`,si.`tender`,si.`is_reverse`, si.`created_by`,  
		MAX(si.`created_on`) AS last_sold_on,
		SUM(sd.`total`) AS total_sale, SUM(sd.`qty`) AS total_qty, 
		SUM(si.`dis_perc`) AS total_dis_perc, SUM(si.`dis_amount`) AS total_dis,
		SUM(si.`tax_perc`) AS total_tax_perc, SUM(si.`tax_amount`) AS total_tax,
...
$ded_returns = 0.00;
$sql_ret = "SELECT sd.`item`, sd.`dcs` ,sd.`description`,  si.`reciept_workstation`, si.`reciept_type`,si.`tender`,si.`is_reverse`, si.`created_by`,  
		MAX(si.`created_on`) AS last_sold_on,
		SUM(sd.`total`) AS total_sale, SUM(sd.`qty`) AS total_qty, 
		SUM(si.`dis_perc`) AS total_dis_perc, SUM(si.`dis_amount`) AS total_dis,
		SUM(si.`tax_perc`) AS total_tax_perc, SUM(si.`tax_amount`) AS total_tax,
		SUM(si.`ship_amount`) AS total_shiping
		 FROM sale_invoice_detail sd 
				...
				WHERE 1=1 ".$where_clause." 
				 GROUP BY sd.`item` ".$having_clause." ORDER BY last_sold_on DESC";
$returns_row = DB::query($sql_ret);
foreach($returns_row as $returns_rows){
  $ded_returns += $returns_rows['total_sale'];  
}                 
$ded_gift = $ded_gift + $gift_gift;

/*echo $ded_gift;
$sub_total = $sub_total+ $gift_cash; */
$total_deductions = $ded_gift+$ded_returns;
$total_earning = $sub_total - $total_deductions;			  
...
$tbl->addCell('');
$tbl->addCell('');
$tbl->addCell('Split Gift Cash: ', '', '');
$tbl->addCell($gift_cash, '', '');*/
...
$tbl->addCell('');
$tbl->addCell('');
$tbl->addCell('Total Sale:  ', '', '');
$tbl->addCell($total_earning, '', '');
...
<?php
                                    ...
                                    $page_url="?route=modules/reports/sale_report";
                                    $export_url = "export_sale_report.php?key=3355";
                                        if(isset($_POST['btnFilter'])){
										...
										$export_rpro = "export_for_rpro.php?key=3355";
                                        if(isset($_POST['btnFilter'])){
                                            foreach ($_POST as $param_name => $param_val) {
                                                if($param_name=='reciept_workstation' || $param_name=='user_id' || $param_name=='tender'|| $param_name=='customer'){
                                                    $param_val = serialize($param_val);
							...
							/* ----------START RPRO-SHOP EXPORT-------- */
										
					$export_rpro_shop = "export_for_rpro_shop.php?key=3355";
                                        if(isset($_POST['btnFilter'])){
                                            foreach ($_POST as $param_name => $param_val) {
                                                if($param_name=='reciept_workstation' || $param_name=='user_id' || $param_name=='tender'|| $param_name=='customer'){
                                                    $param_val = serialize($param_val);
          ...
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">Sale Report</h3>
                                ...
                                <a href='<?php echo $export_url; ?>' target="_blank" class="btn btn-default d-inline pull-right"><i class="glyphicon glyphicon-export"></i>&nbsp;Export- FULL</a>
								<a href='<?php echo $export_rpro; ?>' target="_blank" class="btn btn-danger d-inline pull-right"><i class="glyphicon glyphicon-export"></i>&nbsp;&nbsp;Export - RPRO</a>

<a href='<?php echo $export_rpro_shop; ?>' target="_blank" class="btn btn-warning d-inline"><i class="glyphicon glyphicon-export"></i>&nbsp;&nbsp;Export - RPROSHOP</a>
			</div>
