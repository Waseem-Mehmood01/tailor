...
$reciept_type = isset($_POST['reciept_type']) ? $_POST['reciept_type']: array();
if(empty($reciept_type)){ $reciept_type = isset($_GET['reciept_type']) ? unserialize($_GET['reciept_type']): array(); }
...
if(!empty($reciept_type)){
                                if(is_serialized($reciept_type)){
                                    $reciept_type = unserialize($reciept_type);
                                }
				$reciept_type_array = implode("','", @$reciept_type);

		
		
			$where_clause .=" 
			AND si.`reciept_type` IN ('".$reciept_type_array."') ";
 ...
 */
 ...
 if(@$customer_type <> ''){
 $where_clause .="  AND si.`customer_type`='".@$customer_type."' ";	
    }

//$where_clause .= " AND si.`customer`='Walk-in' ";
...
$tbl->addCell('Invoice Total', '', 'header');
$tbl->addCell('Total Cost', '', 'header');
...
*/
        ...
        SUM(sd.`total_cost`) AS total_cost
		 FROM sale_invoice_detail sd 
				...
				ON(si.`sale_invoice_id`=sd.`sale_invoice_id` AND si.`is_reverse`='0') 
...
if($row['reciept_type']=='Pending'){
	$clr = '#ffeb0033';
} else if($row['reciept_type']=='Paid'){
	$clr = '#00800026';
} else if($row['reciept_type']=='Draft'){
	$clr = '#ff000024';
} else {
	$clr = '';
}
...
$tbl->addRow('',array('style'=>'background:'.$clr));
...
$tbl->addCell(strtoupper($row['customer']));
...
<a class='pull btn btn-default btn-xs' href ='?route=modules/gl/sale_invoice_view&invoice_id=".$row['sale_invoice_id']."'>Detail&nbsp;<span class='glyphicon glyphicon-edit'></span></a>
<button type='button' class='btn btn-primary btn-xs dropdown-toggle' data-toggle='dropdown'>
    More.. <span class='caret'></span></button>
    <ul class='dropdown-menu' role='menu'>
	<li><a class='' href ='?route=modules/gl/sale_invoice_edit&invoice_id=".$row['sale_invoice_id']."'><span class='glyphicon glyphicon-pencil'></span>Edit&nbsp;</a></li>
	<li><a class='' href ='?route=modules/gl/view_all_sale&sale_invoice_id=".$row['sale_invoice_id']."&delete=yes&reciept_type=".$row['reciept_type']."' onclick='return confirm(".$text.");'><span class='glyphicon glyphicon-share-alt'></span>Reverse Sale&nbsp;</a></li> 
	</ul> 
	</div>
");
          ...
          <div class="box">
            <div class="box-header with-border">
              <h4 class=""><?php echo $default_record_msg; ?></h4>
                        <a class="btn btn-primary d-inline" href="?route=modules/gl/view_all_sale">View All of This Month</a>
            ...
            <div class="col-md-4 pull-right">
            	...
            	<tr>
            		...
            		<td><h4>$<?php echo $total; ?> USD</h4></td>
            	</tr>
            	<tr>
            		<td><h4>Total Cost: </h4></td>
            		<td><h4>$<?php echo $cost_total; ?> USD</h4></td>
            	</tr>
            		<tr>
            		<td><h4>Net Profit: </h4></td>
            		<td><h4><strong>$<?php echo ($total-$cost_total); ?> USD</strong></h4></td>
            	</tr>
			...
			<a class="btn btn-success d-inline" href="#"  data-toggle="modal" data-target="#modalFilter"><i class="glyphicon glyphicon-filter"></i>&nbsp;Filter</a>
                        <a class="btn btn-default d-inline pull-right" href="?route=modules/gl/new_invoice"><i class="glyphicon glyphicon-plus"></i>&nbsp;Add New Invoice</a>
            ...
            <div class="box-body">
				<?php  echo $tbl->display(); ?>
                                   ...
                                   <?php 	
					$page_url="?route=modules/gl/view_all_sale";
                                        if(isset($_POST['btnFilter'])){
                                            foreach ($_POST as $param_name => $param_val) {
                                                if( $param_name=='user_id' || $param_name=='tender'|| $param_name=='customer'){
  ...
  <div class="modal fade" id="modalFilter" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
		...
		<form class="form-horizontal" name="frmFilter" id="frmFilter" action="?route=modules/gl/view_all_sale" method="POST">
			...
			<div class="form-group">
               ...
               <div class="form-group">
				<label class="control-label col-sm-3">Reciept Type:</label>
				<div class="col-sm-6">
				  <select class="form-control multiselect" name="reciept_type[]" class="form-control" multiple="multiple">
					
					<option <?php if(in_array('', @$reciept_type)) { echo 'SELECTED'; } ?> value="">-All-</option>
					<option <?php if(in_array('Pending', @$reciept_type)) { echo 'SELECTED'; } ?>  value="Pending">Pending</option>
					<option <?php if(in_array('Draft', @$reciept_type)) { echo 'SELECTED'; } ?>  value="Draft">Draft</option>
					<option <?php if(in_array('Complete', @$reciept_type)) { echo 'SELECTED'; } ?>  value="Complete">Complete</option>	
