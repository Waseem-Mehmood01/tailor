...
@media print{
    ...
    body{
		...
		<table width="100%">
			<tbody>
            <tr>
                <td colspan="2" style="text-align:center;"><h5>LUXURY METAL CARDS</h5>
<p>Luxury Metal Cards <br/>
...
<td>'.date('d-m-Y', strtotime($invoice['date'])).'</td>
			...
			<tr>
			<td>Status: <strong>'.$receipt_type.'</strong></td>
			<td></td>
			</tr>
			<tr>
			<td style="width: 50%;">Customer: '.$invoice['customer'].'
			</td>		
			<td style="width: 50%;">Contact: '.$invoice['customer_phone'].'</td>
			</tr>
			<tr>
			<td>Email: '.$invoice['customer_email'].'</td>
			<td>Address: '.$invoice['address'].'<BR/>'.$invoice['city'].'<BR/>'.$invoice['state'].'</td>
			</tr>
...
$payments = DB::queryFullColumns("SELECT * FROM sale_invoice_payment WHERE sale_invoice_id = %s", $invoice_id);
if($payments <> ''):
echo '<pre>';
...
<BR><BR><BR><BR>
	 <div class="col-md-4 pull-right"> 
		...
		<a class="btn btn-default" target="_BLANK" href="<?php // echo SITE_ROOT."home/export_sale.php?invoice_id=".$invoice_id; ?>"  /><i class="glyphicon glyphicon-export"></i>&nbsp;Export</a>-->
		 <a class="btn btn-primary" href="?route=modules/gl/sale_invoice_edit&invoice_id=<?php  echo $invoice_id; ?>" /><i class="glyphicon glyphicon-pencil"></i>&nbsp;Edit</a> 
