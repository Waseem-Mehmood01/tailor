...
.left>.innerleft {
	...
	background: linear-gradient(to right, rgb(216, 40, 45) 0%,
		rgb(214, 38, 44) 50%, rgb(0, 0, 0) 50%, rgb(0, 0, 0) 100%) !important;
	height: 21px;
	width: 100%;
	border-radius: 100%
}
...
.innercomplete {
	-webkit-print-color-adjust: exact;
	background: linear-gradient(to right, rgb(216, 40, 45) 0%,
		rgb(214, 38, 44) 50%, rgb(216, 40, 45) 50%, rgb(216, 40, 45) 100%)
		!important;
	height: 21px;
	width: 100%;
	border-radius: 100%
...
.innerright {
	-webkit-print-color-adjust: exact;
	background: linear-gradient(to right, rgb(0, 0, 0) 0%, rgb(0, 0, 0) 50%,
		rgb(214, 38, 44) 50%, rgb(214, 38, 44) 100%) !important;
	height: 21px;
	width: 100%;
	border-radius: 100%
}

.completeP, .left, .right {
	border: 2px solid #248440;
	width: 23%;
	height: 37px;
	float: left;
	margin-right: 10px;
	...
	<div class="row">
		<div class="col-xs-12">
			<div class="invoice-title">
				<h2>Invoice</h2>
				<h3 class="pull-right">Order # <?php echo $order['orders_id']; ?></h3>
			</div>
			<hr>
			<div class="row">
				<div class="col-xs-6">
...
echo '<small>[' . $categ . ']</small> ' . $product['name'];

            echo '<small>';
            if ($product['size'] != '') {
                echo '<br><i>Size: </i>' . $product['size'];
            }
            if ($product['color'] != '') {
                echo '<br><i>Color: </i>' . $product['color'];
            }
            if ($product['width'] != '') {
                echo '<br><i>Width: </i>' . $product['width'];
            }
            if ($product['height'] != '') {
                echo '<br><i>Height: </i>' . $product['height'];
            }
            if ($product['prod_note'] != '') {
                echo '<br><i>Note: </i>' . $product['prod_note'];
            }
            echo '</small>';
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">
					<i class="fa fa-credit-card"></i> Card Detail
				</h4>
			</div>
